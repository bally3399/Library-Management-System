...
const BorrowedBooks = () => {
  ...
  // Default fallback borrowed books data
  ...
  useEffect(() => {
    if (!token) {
      console.error("No token found!");
      setError("Unauthorized: No token provided.");
      setBorrowedBooks(defaultBorrowedBooks);
      setLoading(false);
      return;
    }
  ...
  useEffect(() => {
    ...
    const fetchBorrowedBooks = async () => {
      try {
        ...
        setBorrowedBooks(response.data.length > 0 ? response.data : defaultBorrowedBooks);
      } catch (err) {
        console.error("Error fetching books:", err.response?.data || err.message);
        setError("Using default borrowed books due to server issue.");
        setBorrowedBooks(defaultBorrowedBooks);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="px-4 py-8 max-w-xl">
      <h1 className="text-3xl font-bold text-gray-800 mb-6 border-b pb-4">Borrowed Books</h1>

      {borrowedBooks.length === 0 ? (
        <div className="text-center py-10 bg-gray-50 rounded-lg">
          <p className="text-gray-600 text-lg">You don't have any borrowed books at the moment.</p>
        </div>
      ) : (
        <ul className="flex gap-20">
          {borrowedBooks.map((book) => (
            <li
              key={book.id}
              className="bg-white border rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 flex flex-col"
            >
              <img
                src="/milestonehardcover.webp"
                alt={`Cover of ${book.bookTitle}`}
                className="w-1/2 h-8 object-cover rounded-t-lg"
              />
              <div className="p-4 flex flex-col">
                ...
                variant="contained"
                  ...
                  className="mt-4 w-50"
                  disabled={loading}
                  sx={{ backgroundColor: "#917448", margin: "50px", '&:hover': { backgroundColor: "#dd9333" } }}
                >
      ...
      <Modal open={modalOpen} onClose={closeModal}>
        <Box
          ...
          sx={{ outline: "none" }}
        >
          <Typography variant="h6" className="mb-4">Rate and Review</Typography>
          <TextField
            select
            fullWidth
            label="Rating"
      ...
      {error && <p className="mt-4 text-gray-500 text-center">{error}</p>}
      <ToastContainer />
    </div>
  );
};
